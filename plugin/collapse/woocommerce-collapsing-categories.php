...
class Woocommerce_Collapsing_Categories {
	...
	function wa_wcc_deactivation($networkwide) {
	...
	function deactivate_single($multi = FALSE) {
	...
	function wa_wcc_settings_link($actions, $file) {
	...
	$list_categories .= '<script>';

	$list_categories .=  'jQuery(document).ready(function($) {';

	$list_categories .=  '$("li.current-cat-parent").addClass("current-cat");';

	$list_categories .=  'var mtree = $("ul.mtree");';

	$list_categories .=  'mtree.addClass("default");';
	...
	function wa_wcc_mtree_script() {
		...
		wp_register_script('wa_wcc_mtree',get_template_directory_uri().'/plugin/collapse/assets/js/mtree.js',array('jquery'),'',($this->options['configuration']['loading_place'] === 'header' ? false : true));
	...
	function admin_menu_options(){
		add_options_page(
			__('Batik Categories', 'wa_wcc_txt'),
			__('Batik Categories', 'wa_wcc_txt'),
			'manage_options',
	...
	/* category */
	function wa_wcc_posts_category(){
	...
	/* custom css */
	function wa_wcc_custom_css(){
		echo '
		<div id="wa_wcc_custom_css">
			<textarea  name="wa_wcc_settings[custom_css]" placeholder=".wa_wcc_list { color: #ccc !important; }"  >'.esc_attr(isset($this->options['settings']['custom_css']) ? $this->options['settings']['custom_css'] : '').'</textarea>';
		echo '<p class="description">'.__('custom styles or override existing styles to meet your requirements.', 'wa_wcc_txt').'</p></div>';

	}

	/* time out */
	function wa_wcc_duration() {
	...
	function options_page() {
		...
		echo '</h2><div class="wa-wcc-settings"><div class="wa-wcc-credits"><h3 class="hndle">'.__('WooCommerce collapsing categories', 'wa_wcc_txt').'</h3>
					...
					<p class="inner">'.__('Please, ', 'wa_wcc_txt').'<a href="http://wordpress.org/support/view/plugin-reviews/woocommerce-collapsing-categories" target="_blank" title="'.__('rate it', 'wa_wcc_txt').'">'.__('rate it', 'wa_wcc_txt').'</a> '.__('on WordPress.org', 'wa_wcc_txt').'<br />          
						...
						<hr />
					<div style="width:auto; margin:auto; text-align:center;"><a href="http://weaveapps.com/shop/wordpress-plugins/woocommerce-collapsing-categories-wordpress-plugin/" target="_blank"><img width="270" height="70" src="'.get_template_directory_uri().'/plugin/collapse/assets/images/wcc-pro.png"/></a></div>
